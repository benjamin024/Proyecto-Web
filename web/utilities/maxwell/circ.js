...
function lead_getMessage() {
  if (bBothLeadsConnected) {
    ...
    return 'Arrastra las puntas a los nodos para medir su voltaje.';
  }
}
...
function resistor_getMessage() {
  if (!this.isTemplate) {
    return "Resistencia de " + this.value + "&Omega;";
  } else {
    return "Arrastra para agregar una Resistencia";
  }
}
...
function vs_getMessage() {
  if (!this.isTemplate) {
    return "Fuente de alimentación de " + this.value + "V";
  } else {
    return "Arrastra para agregar una fuente de Alimentación.";
  }
}
...
function init() {
  ...
  objects.push(new VoltageSource(20, 100, 5, true));
  objects.push(new Resistor(20, 250, 1, true));
